...
class MySchedulesActivity : AppCompatActivity() {
    ...
    private fun loadSchedules() {
        ...
        db.collection("barbers").document(barberId).collection("availability")
            .get()
            .addOnSuccessListener { documents ->
                if (documents.isEmpty) {
                    ...
                    val schedules = documents.map { doc ->
                        val date = doc.id
                        val timeSlots = doc.get("availableTimes") as? List<String> ?: emptyList()
                        Schedule(date = date, timeSlots = timeSlots)
                    }
